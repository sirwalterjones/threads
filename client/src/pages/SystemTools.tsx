...
const SystemTools: React.FC = () => {
  ...
  const runIngest = async () => {
    try { setError(''); const r = await apiService.ingestWordPressData(); setMsg('Ingestion started'); } catch (e:any) { setError(e?.response?.data?.error || 'Failed to start ingestion'); }
  };
  ...
  return (
    <Box sx={{ p:2 }}>
      ...
      <Stack direction="row" spacing={2}>
        <Button variant="contained" onClick={runIngest}>Sync from WordPress</Button>
