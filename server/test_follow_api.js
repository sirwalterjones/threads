...
async function testFollowAPI() {
  try {
    ...
    try {
      const statusResponse = await axios.post('https://cso.vectoronline.us/api/posts/follow-status', {
        postIds: [1, 2, 3]
      }, {
        headers: {
    ...
    try {
      const followResponse = await axios.post('https://cso.vectoronline.us/api/posts/1/follow', {}, {
        headers: {
          'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE',
          'Content-Type': 'application/json'
        }
    ...
    try {
      const unfollowResponse = await axios.delete('https://cso.vectoronline.us/api/posts/1/follow', {
        headers: {
          'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE'
        }
      });
    ...
    try {
      const followingResponse = await axios.get('https://cso.vectoronline.us/api/posts/following?page=1&limit=5', {
        headers: {
          'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE'
        }
      });
...
console.log('1. Get your JWT token from the browser (localStorage.getItem("token"))');
console.log('2. Replace YOUR_JWT_TOKEN_HERE with your actual token');
console.log('3. Run: node test_follow_api.js');
